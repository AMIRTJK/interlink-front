...
27. Именование интерфейсов пропсов — по месту объявления:
    - Если интерфейс объявлен ВНУТРИ файла компонента (локальные пропсы) — всегда называть `IProps`
    - Если интерфейс объявлен в `model.ts` — полное имя: `IComponentNameProps` (например, `IUserCardProps`)
      Правильно (в файле компонента):
...
34. Всегда используй методологию БЭМ для именования CSS классов (например, `block__element--modifier`).
