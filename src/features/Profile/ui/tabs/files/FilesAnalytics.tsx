...
export const FilesAnalytics = () => {
  const kpiCards = [
    { label: "Всего файлов", value: "245", sub: "В хранилище", color: "indigo" },
    { label: "Занято места", value: "4.2 ГБ", sub: "Из 15 ГБ", color: "emerald" },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-bold text-zinc-700 dark:text-zinc-200">Типы файлов</h3>
          </div>
          <ResponsiveContainer width="100%" height={220}>
            <PieChart>
              <Pie data={mockTypeData} cx="50%" cy="50%" innerRadius={55} outerRadius={95} paddingAngle={3} dataKey="value">
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-bold text-zinc-700 dark:text-zinc-200">Загрузки за полгода (МБ)</h3>
          </div>
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={mockSizeData} barSize={36} margin={{ top: 4, right: 4, left: 0, bottom: 4 }}>
              <XAxis dataKey="name" axisLine={false} tickLine={false} tick={axisTick} />
              <YAxis axisLine={false} tickLine={false} tick={axisTick} width={30} />
              <Tooltip cursor={{ fill: "rgba(0,0,0,0.04)" }} contentStyle={tooltipStyle} formatter={(v: number) => [v + " МБ", "Объем"]} />
