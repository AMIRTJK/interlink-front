...
const normalizeTypeBreakdown = (items: IRawFilesAnalyticsData["type_breakdown"]): IFilesTypeBreakdownItem[] =>
...
const normalizeFolderBreakdown = (breakdown: IRawFilesFolderBreakdown): IFilesFolderBreakdownItem[] => {
...
const normalizeUploadActivity = (items: IRawFilesUploadActivityItem[]): IFilesUploadActivityItem[] =>
  items.map((item) => ({
    month: item.period,
    count: item.file_count,
    total_size: item.total_size,
...
const normalizeAnalyticsData = (raw: IRawFilesAnalyticsData): IFilesAnalyticsData => ({
