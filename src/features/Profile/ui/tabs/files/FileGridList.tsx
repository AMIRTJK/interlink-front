...
import { Pin, Trash2, Archive, FileText, FileSpreadsheet, Image as ImageIcon, File } from "lucide-react";
...
export const FileGridList = ({ files, viewMode, onTogglePin, onDelete }: IProps) => {
  const getCoverContent = (file: IFileItem) => {
    ...
    switch (file.type) {
      ...
      case "document":
      default:
        // md файлы серые, docx/pptx сине-фиолетовые
  ...
  const getListIcon = (type: string) => {
    switch (type) {
      ...
      case "image":
        return <ImageIcon size={18} className="text-cyan-500!" />;
      case "archive":
    ...
    <If is={viewMode === "grid"}>
      ...
      <div className="space-y-2">
        {files.map((file) => (
          <div
            key={file.id}
            className="group flex items-center justify-between p-4 bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 hover:border-slate-200 dark:hover:border-slate-600 shadow-sm transition-all"
          >
            ...
            <div className="flex items-center gap-2">
              <Tooltip title={file.pinned ? "Открепить" : "Закрепить"}>
                <button
                  onClick={() => onTogglePin(file.id)}
                  className={`p-2 rounded-full cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors ${
                    file.pinned ? "text-amber-500!" : "text-slate-400"
                  }`}
                >
                  <Pin size={15} className={file.pinned ? "fill-amber-500!" : ""} />
                </button>
              </Tooltip>
              ...
              <Tooltip title="Удалить">
                <button
                  onClick={() => onDelete(file.id)}
                  className="p-2 rounded-full text-slate-400 hover:text-red-500! hover:bg-red-50 dark:hover:bg-red-950/20 transition-colors cursor-pointer"
                >
                  <Trash2 size={15} />
                </button>
              </Tooltip>
