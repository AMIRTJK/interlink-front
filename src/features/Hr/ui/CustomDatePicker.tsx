...
export const CustomDatePicker = ({ value, onChange, hasError, disabled }: ICustomDatePickerProps) => {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        ...
        <span className={formattedValue ? "text-gray-800 dark:text-slate-200" : "text-gray-400 dark:text-slate-600"}>
        ...
        <div className="absolute z-20 mt-1 w-64 p-3.5 rounded-2xl border border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-xl flex flex-col gap-2">
          <If is={view === "calendar"}>
            <div className="flex items-center justify-between border-b border-gray-100 dark:border-slate-800/80 pb-2">
              <button type="button" onClick={handlePrevMonth} className="p-1 rounded-lg hover:bg-gray-100 dark:hover:bg-slate-800 text-gray-500 dark:text-slate-400 cursor-pointer">
                <ChevronLeft size={16} />
              </button>
              <button
                type="button"
              ...
              <button type="button" onClick={handleNextMonth} className="p-1 rounded-lg hover:bg-gray-100 dark:hover:bg-slate-800 text-gray-500 dark:text-slate-400 cursor-pointer">
            ...
            <div className="grid grid-cols-7 gap-1 text-center">
              {WEEKDAYS.map((d) => (
                <span key={d} className="text-[10px] font-semibold text-gray-400 dark:text-slate-500 py-1">{d}</span>
              ))}
              ...
              {days.map((day, idx) => {
                ...
                return (
                  <div key={idx} className="h-7 w-7 flex items-center justify-center m-auto">
                    <If is={day !== null}>
                      <button
                        type="button"
                        onClick={() => handleSelectDay(day!)}
                        className={`h-7 w-7 flex items-center justify-center rounded-lg text-xs font-semibold cursor-pointer ${
                          isSel ? "bg-indigo-600! text-white!" : "text-gray-700 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-800"
                        }`}
                      >
                        {day}
                      </button>
          ...
          <If is={view === "years"}>
            <div className="flex items-center justify-between border-b border-gray-100 dark:border-slate-800/80 pb-2">
              <button type="button" onClick={() => setYearPage((p) => p - 12)} className="p-1 rounded-lg hover:bg-gray-100 dark:hover:bg-slate-800 text-gray-500 dark:text-slate-400 cursor-pointer">
                <ChevronLeft size={16} />
              </button>
              <span className="text-xs font-bold text-gray-800 dark:text-slate-200">Выбор года</span>
              <button type="button" onClick={() => setYearPage((p) => p + 12)} className="p-1 rounded-lg hover:bg-gray-100 dark:hover:bg-slate-800 text-gray-500 dark:text-slate-400 cursor-pointer">
                <ChevronRight size={16} />
              </button>
            </div>
                ...
                return (
                  <button
                    ...
                    className={`py-1.5 rounded-lg text-xs font-semibold cursor-pointer ${
                      isCurrent ? "bg-indigo-600! text-white!" : "text-gray-700 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-800"
                    }`}
                  >
                    {y}
                  </button>
