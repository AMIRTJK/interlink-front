	...
	switch (priority) {
		case "critical":
			return {
				...
				dotBg: "bg-rose-500",
				pillClass:
					"bg-rose-50 dark:bg-rose-950/40 border-rose-200/70 dark:border-rose-900/50 text-rose-700 dark:text-rose-300",
			};
		case "high":
			return {
				label: "Высокий",
				dotBg: "bg-amber-500",
				pillClass:
					"bg-amber-50 dark:bg-amber-950/40 border-amber-200/70 dark:border-amber-900/50 text-amber-700 dark:text-amber-300",
			};
		case "medium":
			return {
				label: "Средний",
				dotBg: "bg-blue-500",
				pillClass:
					"bg-blue-50 dark:bg-blue-950/40 border-blue-200/70 dark:border-blue-900/50 text-blue-700 dark:text-blue-300",
			};
		case "low":
		default:
			return {
	...
	switch (status) {
		case "overdue":
			return {
				...
				dotBg: "bg-rose-500",
				pillClass:
					"bg-rose-50 dark:bg-rose-950/40 border-rose-200/70 dark:border-rose-900/50 text-rose-600 dark:text-rose-400",
			};
		case "completed":
			return {
				label: "Завершена",
				...
				pillClass:
					"bg-emerald-50 dark:bg-emerald-950/40 border-emerald-200/70 dark:border-emerald-900/50 text-emerald-600 dark:text-emerald-400",
			};
		case "review":
			return {
				label: "На проверке",
				dotBg: "bg-amber-600",
				pillClass:
					"bg-amber-50 dark:bg-amber-950/40 border-amber-200/70 dark:border-amber-900/50 text-amber-700 dark:text-amber-400",
			};
		case "in_progress":
			return {
				label: "В работе",
				dotBg: "bg-blue-500",
				pillClass:
					"bg-blue-50 dark:bg-blue-950/40 border-blue-200/70 dark:border-blue-900/50 text-blue-600 dark:text-blue-400",
			};
		case "new":
		default:
			return {
				...
				dotBg: "bg-sky-500",
				pillClass:
					"bg-sky-50 dark:bg-sky-950/40 border-sky-200/70 dark:border-sky-900/50 text-sky-600 dark:text-sky-400",
			};
	}
};
...
export const TaskListView = ({
	...
	const rangeStart =
		pagination.total === 0 ? 0 : (safePage - 1) * pagination.perPage + 1;
														...
														<div className="flex items-center -space-x-2 shrink-0">
															{task.assignees.slice(0, 2).map((a, i) => (
																<Avatar
																	key={a.id || i}
																	colleague={a}
																	className="w-8 h-8 text-xs font-bold ring-2 ring-white dark:ring-slate-800"
																/>
															))}
