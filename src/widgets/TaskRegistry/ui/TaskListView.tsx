...
interface TaskListViewProps {
  /** Задачи текущей страницы, уже отфильтрованные и отсортированные сервером. */
  tasks: Task[];
...
export const TaskListView = ({
  ...
  const rangeEnd = (safePage - 1) * pagination.perPage + pagedTasks.length;
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="bg-white/40 dark:bg-slate-900/40 backdrop-blur-3xl border border-white/20 dark:border-white/10 rounded-[2.5rem] shadow-none overflow-hidden flex flex-col min-h-[500px]"
    >
      {/* Table */}
                ...
                pagedTasks.map((task, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-6 py-5 text-xs font-mono font-black text-slate-400 group-hover:text-slate-800 dark:group-hover:text-slate-200 transition-colors">
                        {task.id}
                      </td>
