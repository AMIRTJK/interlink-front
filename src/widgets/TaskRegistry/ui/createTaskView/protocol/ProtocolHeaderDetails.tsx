      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
        ...
        <div ref={chairmanRef} className="flex flex-col gap-1.5">
          <label className="text-[10px] font-black uppercase tracking-wider text-[#636e9c] dark:text-purple-300/60 block h-3.5">
            ПРЕДСЕДАТЕЛЬ {selectedChairmanColleagues.length > 0 && `(${selectedChairmanColleagues.length})`} {!isView && <span className="text-red-500">*</span>}
          </label>
          {isView ? (
            <div className="min-h-12 flex flex-wrap items-center gap-2 px-4 py-2 bg-gradient-to-br from-white/95 to-[#d9e0f2]/40 border border-white/90 rounded-2xl shadow-[0_4px_16px_rgba(100,105,240,0.06)]">
              {selectedChairmanColleagues.length > 0 ? (
                selectedChairmanColleagues.map((c) => (
                  <span key={c.id} className="inline-flex items-center gap-2 py-0.5">
                    <Avatar colleague={c} className="w-6 h-6 text-[9px]" allowPreview={false} />
                    <span className="text-xs font-bold text-[#1e2548] dark:text-slate-100">{c.name}</span>
                    {c.role && <span className="text-[10px] text-slate-400 font-normal">({c.role})</span>}
                  </span>
                ))
              ) : (
                <span className="text-xs text-slate-400 italic">Не указан</span>
              )}
            </div>
          ) : (
        ...
        <div ref={participantsRef} className="flex flex-col gap-1.5">
          ...
          {isView ? (
            <div className="min-h-12 flex flex-wrap items-center gap-1.5 px-4 py-2 bg-gradient-to-br from-white/95 to-[#d9e0f2]/40 border border-white/90 rounded-2xl shadow-[0_4px_16px_rgba(100,105,240,0.06)]">
              {batchGlobal.participants.length > 0 ? (
                batchGlobal.participants.map((id) => {
                  ...
                  return (
                    <Tooltip key={id} title={`${col.name} — ${col.role || "Сотрудник"}`}>
                      <span className="inline-flex items-center gap-1.5 pl-1.5 pr-2.5 py-1 bg-white dark:bg-slate-800 border border-slate-200/80 dark:border-white/10 rounded-full shadow-2xs text-xs font-bold text-[#1e2548] dark:text-slate-100">
                ...
                <span className="text-xs text-slate-400 italic">Нет участников</span>
              )}
            </div>
          ) : (
            <>
        ...
        <div className="flex flex-col gap-1.5">
          <label className="text-[10px] font-black uppercase tracking-wider text-[#636e9c] dark:text-purple-300/60 block h-3.5">
            ДАТА {!isView && <span className="text-red-500">*</span>}
          </label>
          {isView ? (
            <div className="h-12 px-4 bg-gradient-to-br from-white/95 to-[#d9e0f2]/40 dark:bg-slate-900/90 border border-white/90 dark:border-white/10 rounded-2xl flex items-center text-xs font-bold text-[#1e2548] dark:text-slate-100 shadow-[0_4px_16px_rgba(100,105,240,0.06)]">
              {batchGlobal.date ? dayjs(batchGlobal.date).format("DD.MM.YYYY") : <span className="text-slate-400 font-normal italic">Не указана</span>}
            </div>
          ) : (
            <ConfigProvider
              theme={{
        ...
        <div className="flex flex-col gap-1.5">
          ...
          {isView ? (
            <div className="h-12 px-4 bg-gradient-to-br from-white/95 to-[#d9e0f2]/40 border border-white/90 rounded-2xl flex items-center text-xs font-bold text-[#1e2548] shadow-[0_4px_16px_rgba(100,105,240,0.06)]">
              {batchGlobal.number ? `№ ${batchGlobal.number}` : <span className="text-slate-400 font-normal italic">Без номера</span>}
            </div>
          ) : (
            <input
              type="text"
