...
const ROLE_CHIP_STYLE_MAP: Record<string, { border: string; bg: string; text: string; dot: string }> = {
  super_admin: {
...
export const UsersTab = () => {
  ...
  const roles = useMemo(() => {
    const raw = (rolesData?.data?.data || rolesData?.data || rolesData || []) as { id: number; name: string }[];
    return raw.map((r) => r.name);
  }, [rolesData]);

  const rolesList = useMemo(() => {
    const raw = (rolesData?.data?.data || rolesData?.data || rolesData || []) as {
      id: number;
      name: string;
  ...
  const departments = useMemo(() => {
    const raw = (deptsData?.data?.data || deptsData?.data || deptsData || []) as { id: number; name: string }[];
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-400 font-medium">Управление сотрудниками и доступами</p>
        </div>
        <button
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <button
          ...
          className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-colors flex items-center gap-1.5 ${
            filters.status === "all" ? "bg-slate-200 text-slate-800" : "bg-slate-50 text-slate-500 hover:bg-slate-100"
          }`}
        >
          <span>Всего</span>
        ...
        <button
          ...
          className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-colors flex items-center gap-1.5 ${
            filters.status === "active" ? "bg-emerald-100 text-emerald-800" : "bg-slate-50 text-slate-500 hover:bg-slate-100"
          }`}
        >
        ...
        <button
          ...
          className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-colors flex items-center gap-1.5 ${
            filters.status === "inactive" ? "bg-slate-200 text-slate-700" : "bg-slate-50 text-slate-500 hover:bg-slate-100"
          }`}
        >
        ...
        <button
          ...
          className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-colors flex items-center gap-1.5 ${
            filters.status === "blocked" ? "bg-rose-100 text-rose-800" : "bg-slate-50 text-slate-500 hover:bg-slate-100"
          }`}
        >
            ...
            >
              <span
                className={`w-1.5 h-1.5 rounded-full transition-all duration-150 ${
                  isSelected ? `${activeStyle.dot} opacity-100 scale-100` : "bg-slate-400 opacity-0 scale-50"
                }`}
              />
              <span>{role}</span>
            </button>
      ...
      <EmployeeFormModal open={isFormOpen} onClose={() => setIsFormOpen(false)} employee={editingUser} />
    </div>
  );
};
