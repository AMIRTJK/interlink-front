...
export const normalizeAccessUsers = (raw: IAdminUser[]): IAccessUser[] => {
  return (Array.isArray(raw) ? raw : []).map((u) => ({
    id: u.id,
    fullName: u.full_name || `${u.last_name || ""} ${u.first_name || ""}`.trim() || "Без имени",
    email: u.email || "—",
...
export const getAccessTableColumns = ({
  ...
  },
  {
    ...
    dataIndex: "email",
    key: "email",
    render: (email) => <span className="text-slate-400 font-normal">{email}</span>,
  },
  {
    title: "ОТДЕЛ",
    dataIndex: "department",
    key: "department",
    render: (dept) => <span className="text-slate-700 font-medium">{dept}</span>,
  },
  {
    title: "РОЛИ",
    dataIndex: "roles",
