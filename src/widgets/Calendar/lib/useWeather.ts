...
export const mapWeatherCode = (
	code: number,
): { description: string; weatherType: WeatherType } => {
	if (code === 0) {
...
const executeWeatherFetch = async (
	...
	try {
		...
		if (json?.daily?.time && Array.isArray(json.daily.time)) {
			json.daily.time.forEach((dateStr: string, index: number) => {
				...
				const maxTemp = Math.round(
					json.daily.temperature_2m_max?.[index] ?? 20,
				);
		...
		if (json?.current) {
			...
			const { description, weatherType } = mapWeatherCode(
				json.current.weather_code ?? 0,
			);
			currentInfo = {
				temp: formattedTemp,
				description,
	...
	activeWeatherPromise = new Promise<IWeatherState>((resolve) => {
		if ("geolocation" in navigator) {
			navigator.geolocation.getCurrentPosition(
				(pos) => {
					executeWeatherFetch(pos.coords.latitude, pos.coords.longitude).then(
						resolve,
					);
				},
				() => {
					executeWeatherFetch(38.56, 68.78).then(resolve); // Dushanbe
				},
				{ timeout: 4000 },
			);
		} else {
...
export const useWeather = () => {
	...
	const { current: currentWeather, dailyMap, isLoading } = weatherState;
	...
	return useMemo(
		...
		[currentWeather, getWeatherForDate, isLoading],
	);
};
