...
const LICENSE_KEY =
  "eyJhbGciOiJFUzI1NiJ9..2Bwabhf7OdSDQflEIS850qfmNw15uD6X9kyfZY3pTb2Zrq2u_ZH-xluxClzHjHli_6h6U7C1ZZkzyveskdJy_Q";

const CLOUD_SERVICES_TOKEN_URL =
  "https://746w835dlnh5.cke-cs.com/token/dev/5632e52c88b84d710137e1d5162106114e2f8ccb6671827d02a15d0d4b8a?limit=10";
...
export const Editor = forwardRef<EditorHandle, EditorProps>(
  (
    ...
    // 3. Ссылка для хранения самого инстанса CKEditor
    const editorInstanceRef = useRef<DecoupledEditor | null>(null);

    // 4. Реализуем метод insertHtml для родителя
    useImperativeHandle(ref, () => ({
      insertHtml: (htmlContent: string) => {
        const editor = editorInstanceRef.current;
        if (editor) {
          editor.model.change((writer) => {
            // Преобразуем HTML строку в модель редактора
    ...
    const { editorConfig } = useMemo<{
      ...
      let baseToolbarItems: any = [];
      if (isReadPage) {
        baseToolbarItems = ["previousPage", "nextPage", "|", "exportPdf"];
      }

      if (!isReadPage) {
        baseToolbarItems.push(
          "undo",
          "redo",
          "|",
          "previousPage",
