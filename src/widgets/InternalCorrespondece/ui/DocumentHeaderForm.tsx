...
import { If, SelectField } from "@shared/ui";
...
import { RecipientSelectionArea } from "./RecipientSelectionArea";
import { Recipient, SelectionMode } from "./DocumentHeaderFormTypes";

export type { Recipient, SelectionMode };
      ...
      >
            ...
            {isIncoming && creator ? (
              ...
              <Form.Item noStyle shouldUpdate>
                {() => {
                  ...
                  return (
                    <RecipientSelectionArea
                      list={selectedRecipients}
                      mode="recipients"
                      emptyText="Получатель"
                      emptyIcon={<UserOutlined style={{ fontSize: "14px" }} />}
                      hasError={hasError}
                      ...
                      onOpenSelector={setActiveSelectorMode}
                      onOpenViewer={setActiveViewerMode}
                    />
                  );
                }}
              </Form.Item>
        ...
        <div className="flex items-center gap-4 mt-1 min-h-[32px]">
          <RecipientSelectionArea
            list={selectedCC}
            mode="copy"
            emptyText={`Копия ${selectedCC.length === 0 ? "(Не указано)" : "(необязательно)"} `}
            emptyIcon={<CopyOutlined style={{ fontSize: "14px" }} />}
            isDarkMode={isDarkMode}
            isIncoming={isIncoming}
            isReadOnly={isReadOnly}
            chipClass={chipClass}
