...
export const ChatHeader = ({
  ...
  // Во всплывающем окне (там есть «развернуть» и «закрыть») в макетах слева/справа
  // блок сужен до ширины панели бесед — там группа кнопок ставится по центру. В
  // модуле «Чат» этих двух кнопок нет, в макетах сверху/снизу блок во всю ширину —
  // и там всё остаётся как было: группа прижата к правому краю.
  const hasWindowControls = Boolean(onToggleExpand || onRequestClose);
  const isVerticalLayout = layout === "left" || layout === "right";
  const isCentered = isVerticalLayout && hasWindowControls;
