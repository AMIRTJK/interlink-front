...
/**
 * Обёртка холста версии: карточка автора при наведении на подсвеченный
 * фрагмент. Подсветка тут лежит прямо в разметке холста, поэтому хватает
 * обычного ховера — в отличие от основного холста, где она живёт отдельным
 * слоем под текстом (AuthorshipOverlay).
 *
 * Состояние ховера держим здесь, а не в родителе: оно меняется на каждое
 * движение мыши, и перерисовываться должна только эта колонка.
 */
