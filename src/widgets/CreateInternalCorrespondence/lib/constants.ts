...
// ===== Тело документа =====
// Базовый кегль холста: размер, которым рисуется текст без собственного
// font-size. Предпросмотр, печать, эскизы навигации и сравнение версий обязаны
// брать именно его. Состояние `fontSize` в редакторе для этого не годится —
// оно показывает размер ПОД КАРЕТКОЙ (обновляется при каждом движении курсора),
// и документ печатался кеглем случайного слова, из-за чего текст расходился
// с холстом.
