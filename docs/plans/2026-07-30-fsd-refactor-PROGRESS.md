...
- **Последнее обновление:** 2026-08-01 — Идёт XL модуля 12 (`CreateInternalCorrespondence.tsx`): срезы A, B и C закрыты целиком, из среза D закрыты части 1–2 (история undo/redo, перетаскивание ЭЦП, импорт Word, печать, скролл-follow, версии, сохранение черновика), часть 3a — **пагинация** (`paginateEditorDom.ts`, `editorPageSplitters.ts`, `editorCaretKeeper.ts`, `editorSplitGroup.ts`) и часть 3b — **команды правки страниц и буфер обмена** (`editorPageOps.ts`, `useEditorCommands.ts`, `useEditorClipboard.ts`, `editorFragments.ts`). 7256 → **3395**, 36 новых файлов, все ≤246 строк. Шаг остаётся открытым: до `return` осталось ~2700 строк, главный оставшийся узел — `handleEditorKeyDown` (~390). Выносить только отдельным заходом с проверкой инвариантов каретки.
...
- [ ] `XL` 7256 → `widgets/CreateInternalCorrespondence/ui/CreateInternalCorrespondence.tsx` — под-план ниже
  ...
  - [ ] **срез D, часть 3c — остаток ядра (~2700 строк до `return`).** Осталось главное: `handleEditorKeyDown` (~390 строк, один `useCallback`) — Enter/Tab/Backspace/Delete/стрелки через границу листа, слияние блоков и разведение AUTOSPLIT-групп. Брать отдельным заходом и с проверкой инвариантов из `editor-pagination-architecture` (AUTOSPLIT, структурные разрезы, синхронная пагинация после `innerHTML`, гибридная каретка). Более мелкое, что ещё окупится: кластер подписей/согласований (`signaturesConfirm` + `approvalsConfirm`, ~150 — упирается в TDZ: `saveDraft` и `resetHistory` объявлены по разные стороны от него), выбор и загрузка версии в редактор (`handleSelectVersion` + эффект синхронизации, ~90)
