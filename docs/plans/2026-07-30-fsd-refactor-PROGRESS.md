...
- **Последнее обновление:** 2026-08-01 — Идёт XL модуля 12 (`CreateInternalCorrespondence.tsx`): срезы A, B и C закрыты целиком, из среза D закрыты части 1–2 (история undo/redo, перетаскивание ЭЦП, импорт Word, печать, скролл-follow, версии, сохранение черновика), часть 3a — **пагинация** (`paginateEditorDom.ts`, `editorPageSplitters.ts`, `editorCaretKeeper.ts`, `editorSplitGroup.ts`) часть 3b — **команды правки страниц и буфер обмена** (`editorPageOps.ts`, `useEditorCommands.ts`, `useEditorClipboard.ts`, `editorFragments.ts`) и часть 3c — **клавиатура** (`editorKeyEnter.ts`, `editorKeyTab.ts`, `editorKeyArrows.ts`, `editorKeyDelete.ts`, `editorKeyModel.ts`, `useEditorKeyDown.ts`). 7256 → **2991**, 42 новых файла, все ≤246 строк. Ядро редактора разобрано полностью. Шаг остаётся открытым: до `return` осталось ~2300 строк, но крупных монолитов там больше нет — дальше средние кластеры (подписи/согласования, версии, вложения) и остаток JSX.
...
- [ ] `XL` 7256 → `widgets/CreateInternalCorrespondence/ui/CreateInternalCorrespondence.tsx` — под-план ниже
  - [x] срезы A+B, часть 1: линейка полей и сетка страницы → `ui/createInternalCorrespondence/` (7256 → **6923**)
  - [x] срезы A+B, часть 2a: наборы тегов + предикаты узлов (`editorTags.ts`), хелперы табуляции и каретки-в-табуляторе (`editorTabs.ts`) (6923 → **6749**)
  - [x] срезы A+B, часть 2b: блок-хелперы каретки, слияние блоков, снимок каретки и чистка HTML, цитата пересылки (6749 → **6228**)
  - [x] срез B, часть 3: импорт docx (`jcToAlign`, `twipsToPx`, `analyzeDocxFormatting`, `makeImportPageBreak`, `liftPageBreakMarker`) → `docxImport.ts` (вошло в тот же проход)
  - [x] хвост срезов A+B: `DS_STAMP_*` + `buildStampQRSvg` → `stampGeometry.ts`, иконка → `FileTextIcon.tsx` (6228 → **6183**). Перед компонентом остался только `let splitGroupSeq = 0` — выносить вместе с логикой авторазбиения
  - [x] **срез C, часть 1: липкая шапка редактора + холст листа.** Тулбар разбит на 4 группы (`ToolbarFormatGroup`, `ToolbarParagraphGroup`, `ToolbarPageGroup`, `ToolbarViewToggles`), «цилиндры» разделов → `SectionCylindersBar`, две закреплённые пагинации → `IncomingPagerBar` / `VersionComparePagerBar` (всё в `ui/createInternalCorrespondence/toolbar/`), лист страницы → `EditorPageSheet.tsx`, плейсхолдер ЭЦП → `StampPlaceholder.tsx` (6183 → **5615**)
  - [x] **срез C, часть 2: шапка экрана, оверлей ЭЦП и форма реквизитов.** `ScreenActionsBar.tsx` (панель действий: назад / просмотр / печать / сохранить / отклонить / отменить подпись / отправить), `StampZoomOverlay.tsx` (портальный просмотр вшитого штампа), новая подпапка `documentMeta/`: `model.ts`, `DocumentMetaToggle.tsx`, `LetterTypeSelect.tsx`, `ImportanceSelect.tsx`, `RecipientField.tsx` (общее поле «Кому»/«Копия»), `AttachmentsField.tsx`; плюс `EditorSurface.tsx` (сам `contentEditable`) (5615 → **5120**)
  - [ ] **срез C, часть 3 — что осталось в JSX (~690 строк на `return`).** Крупных самодостаточных блоков больше нет. Остаток: группа боковых панелей `panelsGroupRef` (~110 строк, но ~45 пропсов — выносить только вместе с объединением состояния панелей в один объект), холст страницы с линейкой и боковыми канвасами (~180, плотно завязан на рефы и геометрию), поле «Тема» (14) и хвост модалок — по отдельности не окупаются
  - [x] **срез D, часть 1: изолированные хуки и печать.** `useEditorHistory.ts` (собственный стек undo/redo), `useStampDrag.ts` (перетаскивание и масштабирование плейсхолдера ЭЦП), `useWordImport.ts` (импорт .docx кнопкой и перетаскиванием), `printDocument.ts` (CSS листа и печать через скрытый iframe), `mammothToEditorHtml` → `docxImport.ts` (5120 → **4632**)
  - [x] **срез D, часть 2: скролл-follow, версии, черновик, вшитый штамп.** `scrollFollowModel.ts` + `useCanvasScrollFollow.ts` + `usePaneScrollFollow.ts` (четыре «пришвартованных» блока), `versionsLib.ts` (нормализация версий с бэкенда), `useDraftMutations.ts` (пять мутаций сохранения + `saveDraft`), `buildEmbeddedStampHtml` → `stampGeometry.ts` (4632 → **4267**)
  - [x] **срез D, часть 3a: пагинация.** `paginateEditor` (508 строк одним `useCallback`) вынесен целиком: `paginateEditorDom.ts` (главный проход — сборка разрезанных блоков, ранний выход, заворачивание голых узлов, цикл раскладки по листам), `editorPageSplitters.ts` (`makeSpacer` + фабрика `createBlockSplitters`: таблица по строкам, список по пунктам, абзац по бюджету), `editorCaretKeeper.ts` (структурный снимок каретки + гибридное восстановление), `editorSplitGroup.ts` (общий счётчик AUTOSPLIT-групп, которым пользуются и пагинация, и разрез по Enter). В компоненте остался обёрточный `useCallback` на 10 строк: геометрия листа + запись `lastPaginatedHeightRef` (4267 → **3760**)
  - [x] **срез D, часть 3b: команды правки страниц и буфер обмена.** `editorPageOps.ts` (чистые DOM-операции: `insertPageBreakAtCaret`, `removePageBlocks`, `insertFragmentIntoEditor`), `useEditorCommands.ts` (три команды-обёртки: фиксация истории → правка DOM → синхронная перепагинация), `editorFragments.ts` (`buildFragmentFromHtml` / `buildInlineFragmentFromHtml` — были `useCallback` с пустыми зависимостями, стали обычными функциями), `useEditorClipboard.ts` (вставка с очисткой Word-разметки, копирование/вырезание очищенного фрагмента и оба нативных слушателя). Эффект-сторож `pageToDelete >= pageCount` остался в компоненте — он про состояние экрана, не про DOM (3760 → **3395**)
  - [x] **срез D, часть 3c: клавиатура редактора.** `handleEditorKeyDown` (388 строк одним `useCallback`) разобран на пять независимых веток: `editorKeyEnter.ts` (Shift+Enter — мягкий перенос; Enter — выход из пустого пункта списка и собственный разрез AUTOSPLIT-блока), `editorKeyTab.ts` (уровень пункта, удаление табулятора, вставка выделяемого табулятора), `editorKeyArrows.ts` (переход каретки через распорку между листами), `editorKeyDelete.ts` (Backspace/Delete: снятие ручного разрыва и слияние блоков через границу), `editorKeyModel.ts` (тип обработчика и контракт «обработал — верни true»), `useEditorKeyDown.ts` (Ctrl+Z/Y плюс диспетчер по списку обработчиков). Порядок веток и все ранние выходы сохранены один в один (3395 → **2991**)
  - [ ] **срез D, часть 3d — остаток (~2300 строк до `return`).** Крупных монолитов больше нет, дальше — средние кластеры: подписи/согласования (`signaturesConfirm` + `approvalsConfirm`, ~150 — упирается в TDZ: `saveDraft` и `resetHistory` объявлены по разные стороны от него), выбор и загрузка версии в редактор (`handleSelectVersion` + эффект синхронизации, ~90), вложения (`handleFileChange` + `applyFinalDS` / `applyApproverDS` / работа с согласующими, ~150), цитата пересылки (`forwardSyncedModeRef` + эффект, ~55). Плюс срез C, часть 3 (JSX): группа боковых панелей и холст страницы — они окупятся только вместе с объединением состояния панелей в один объект
...
|---|---|---|---|---|
| 2026-08-01 | `widgets/CreateInternalCorrespondence/ui/CreateInternalCorrespondence.tsx` | **срез D, часть 3c — клавиатура редактора.** 6 новых файлов: `editorKeyEnter.ts` (140 — `handleSoftBreakKey`, `handleEnterKey`), `editorKeyDelete.ts` (129 — `handleDeleteKeys` + `collectBoundary` и `isMergeableStop`), `editorKeyTab.ts` (83 — `handleTabKey`), `editorKeyArrows.ts` (69 — `handleArrowKey`), `useEditorKeyDown.ts` (73 — Ctrl+Z/Y и диспетчер по `KEY_HANDLERS`), `editorKeyModel.ts` (21 — `IEditorKeyDeps`, `TEditorKeyHandler`). Убраны 19 ставших ненужными импортов (`TAB_STEP_CM`, `closestLiOf`, `deleteTabBeforeCaret`, `getTextIndentCm`, `makeTabSpacer`, `tabNbspCount`, `topLevelBlockOf`, `caretAtBlockStart`, `caretAtBlockEnd`, `blockAcrossPageBoundary`, `mergeAcrossBoundary`, `charPosAt`, `nextSplitGroupId`, `AUTOSPLIT_ATTR`, `PAGE_SPLITTABLE_TAGS`, `EDITOR_ATOMIC_TAGS`, `isSpacerNode`, `isStampNode`, `isPageBreakNode`) | 3395 → **2991** (макс. новый — 140) | на ревью |
| 2026-08-01 | `widgets/CreateInternalCorrespondence/ui/CreateInternalCorrespondence.tsx` | **срез D, часть 3b — команды правки страниц и буфер обмена.** 4 новых файла: `editorPageOps.ts` (220 — `insertPageBreakAtCaret`, `removePageBlocks`, `insertFragmentIntoEditor`: чистые операции над DOM редактора), `useEditorCommands.ts` (99 — `insertPageBreak` / `deletePage` / `insertFragmentAtCaret`, у всех один сценарий «зафиксировать историю → правка → перепагинация»), `useEditorClipboard.ts` (131 — `handleEditorPaste`, `handleEditorCopyCut` и оба нативных слушателя `paste`/`copy`/`cut`), `editorFragments.ts` (40 — сборка фрагментов из HTML, блочная и инлайновая). Убраны ставшие ненужными импорты (`sanitizeWordHtml`, `htmlToPlainText`, `EDITOR_BLOCK_TAGS`) | 3760 → **3395** (макс. новый — 220) | на ревью |
| 2026-08-01 | `widgets/CreateInternalCorrespondence/ui/CreateInternalCorrespondence.tsx` | **срез D, часть 3a — ядро пагинации.** 4 новых файла в `ui/createInternalCorrespondence/`: `paginateEditorDom.ts` (199 — главный проход разбивки), `editorPageSplitters.ts` (226 — `makeSpacer` + `createBlockSplitters`: таблицы по строкам, списки по пунктам, абзацы по бюджету с бинарным поиском), `editorCaretKeeper.ts` (140 — `createCaretKeeper`: структурный снимок каретки + символьный fallback), `editorSplitGroup.ts` (7 — `nextSplitGroupId()` вместо модульной `let splitGroupSeq`). В компоненте `paginateEditor` стал обёрткой на 10 строк. Убраны ставшие ненужными импорты (`truncateToChars`, `dropChars`, `brAtCharBoundary`, `removeLeadingBr`, `removeSpacerSafely`, `getCaretCharOffset`, `restoreCaretCharOffset`) | 4267 → **3760** (макс. новый — 226) | на ревью |
...
| 2026-07-31 | `widgets/CreateInternalCorrespondence/ui/CreateInternalCorrespondence.tsx` | **XL, срезы A+B, часть 1 из 4.** Новая папка `ui/createInternalCorrespondence/`: `docLayout.ts` (`PX_PER_CM`, `RULER_*`, тип `DocLayout`, `DEFAULT_DOC_LAYOUT`, `pageWidthForOrientation`, `normalizeDocLayout`, `hasDefaultRulerMargins`, `parseDocLayoutAttr`, `withDocLayout`, `splitDocLayout`, `stripDocLayout`), `EditorRuler.tsx` (горизонтальная линейка с перетаскиванием полей), `PageGrid.tsx` (константы сетки, `snapToGrid`, `GRID_SNAP_STEP`, компонент сетки). Блок оказался самодостаточным: наружу нужны 13 символов, внутрь — только тип `PageOrientation`. Далее часть 2a: `editorTags.ts` (`EDITOR_BLOCK_TAGS`, `EDITOR_ATOMIC_TAGS`, `PAGE_SPLITTABLE_TAGS`, `isSpacerNode`, `isPageBreakNode`, `isStampNode`), `editorTabs.ts` (`WORD_BOUNDARY_RE`, `TAB_STEP_CM`, `NBSP`, замер ширины табулятора через canvas, `makeTabSpacer`, `nodeBeforeCaret`/`nodeAfterCaret`, `tabSpacerToDelete`, `removeTabSpacer`, `closestLiOf`, `getTextIndentCm`, `deleteTabBeforeCaret`). Части 2b и 3: `editorBlocks.ts` (`topLevelBlockOf`, `blockAcrossPageBoundary`, `rangeHasContent`, `caretAtBlockStart/End`), `editorMerge.ts` (`EMPTY_INLINE_TAGS`, `normalizeBlock`, `mergePageBlocks`, `mergeAcrossBoundary`), `editorCaret.ts` (снимок каретки `getCaretCharOffset`/`restoreCaretCharOffset`/`charPosAt`, `removeSpacerSafely`, `wrapBareTopLevelNodes`, `htmlToPlainText`, `cleanEditorArtifacts`), `forwardQuote.ts` (цитата пересылаемого письма), `docxImport.ts` (маркер разрыва из Word, `jcToAlign`, `twipsToPx`, `analyzeDocxFormatting` с динамическим `jszip`). Итог по файлу: diff 61 вставка / 1089 удалений, ни одной правки внутри тела компонента | 7256 → **6228** (макс. новый — 182) | на ревью |
| 2026-07-31 | `widgets/CreateInternalCorrespondence/ui/CreateInternalCorrespondence.tsx` | **XL, срез C, часть 1.** Липкая шапка редактора и холст листа разобраны на подкомпоненты. Новая подпапка `ui/createInternalCorrespondence/toolbar/`: `ToolbarFormatGroup.tsx` (undo/redo, заголовки, выпадашка кегля, начертания), `ToolbarParagraphGroup.tsx` (выравнивание, списки, горизонтальная линия), `ToolbarPageGroup.tsx` (разрыв страницы, ориентация, импорт Word), `ToolbarViewToggles.tsx` (галочки «Линейка / Сетка / Область навигации / Панель разделов сверху / Режим просмотра входящего письма / Режим просмотра истории версий» + кнопка сброса полей), `SectionCylindersBar.tsx` («цилиндры» разделов под тулбаром), `IncomingPagerBar.tsx` и `VersionComparePagerBar.tsx` (две закреплённые полосы пагинации). Рядом в `ui/createInternalCorrespondence/`: `EditorPageSheet.tsx` (белый лист, номер страницы, подтверждение удаления страницы), `StampPlaceholder.tsx` (перетаскиваемый плейсхолдер ЭЦП с угловым ресайзером). Разметка перенесена дословно; заменены только чтения родительских переменных на пропсы: `attachments.length` → `attachmentsCount`, `!!id` → `hasSectionsToggle`, `panelMode && panelSource` → `hasIncomingSource`, `allVersions.length > 0` → `hasVersions`, `panelSource.id` → `sourceId`, `latestVersion?.versionNumber` / `activeVersion?.versionNumber` / `activeVersion?.date` → примитивы, `PAGE_STRIDE` / `PAGE_HEIGHT` / `PAGE_PAD_V` → `pageStride` / `pageHeight` / `pagePadV`. Из главного файла ушли 18 иконок `lucide-react`, `TBtn`, `FONT_SIZES`, `DSStamp` | 6183 → **5615** (макс. новый — 197) | на ревью |
| 2026-07-31 | `widgets/CreateInternalCorrespondence/ui/CreateInternalCorrespondence.tsx` | **XL, срез C, часть 2.** `ScreenActionsBar.tsx` — панель действий экрана (назад, предпросмотр, печать, сохранить, отклонить, отменить подпись, отправить); составные обработчики остались у владельца и передаются колбэками `onPreview` / `onSend` / `onCancelSign`. `StampZoomOverlay.tsx` — портальный просмотр вшитого штампа в полном размере (`createPortal` переехал внутрь компонента, наружу — только `src` и `onClose`). `EditorSurface.tsx` — сам `contentEditable`-холст письма вместе со своим километровым `className`. Новая подпапка `documentMeta/`: `model.ts` (`ILetterTypeOption`, `IImportanceOption`), `DocumentMetaToggle.tsx` (шапка-аккордеон «Реквизиты документа»), `LetterTypeSelect.tsx`, `ImportanceSelect.tsx`, `AttachmentsField.tsx` и `RecipientField.tsx` — **общее** поле получателей: блоки «Кому» и «Копия» были построчными дублями, различия вынесены в пропсы (`wrapperClassName`, `label`, `placeholder`, обработчики ввода), трейлинг-кнопка «+ Копия» отдана через `children`, выпадашка — через слот `dropdown` (так `UserDropdown` остаётся объявленным в родителе и его поведение не меняется). Заодно сняты пять мёртвых импортов иконок, неиспользуемых ещё до кампании (`Search`, `Plus`, `User`, `Download`, `UserPlus`) | 5615 → **5120** (макс. новый — 166) | на ревью |
| 2026-07-31 | `widgets/CreateInternalCorrespondence/ui/CreateInternalCorrespondence.tsx` | **XL, срез D, часть 1 — первые хуки.** `useEditorHistory.ts` — собственный стек undo/redo целиком (снимки чистого HTML + каретка в символах, склейка серии набора по паузе, перехват `historyUndo`/`historyRedo` из меню браузера); наружу отдаёт `canUndo`, `canRedo`, `undoEdit`, `redoEdit`, `resetHistory`, `commitHistoryNow`, `scheduleHistoryCommit`, внутрь берёт `editorRef`, `paginateEditorRef`, `getCleanEditorHtml`, `setPageCount`, `setEditorContent`. `useStampDrag.ts` — перетаскивание плейсхолдера ЭЦП с привязкой к сетке и масштабирование за угловой маркер; рефы `isDraggingStamp` и `dragOffset` переехали внутрь хука. `useWordImport.ts` — `importWordFile` + кнопка + drag&drop вместе со своими состояниями `importingWord` / `isDraggingWord`. `printDocument.ts` — `buildPrintPageCss` и `printDocumentPages` (печать через скрытый iframe); `handlePrint` в компоненте сжался до сбора параметров. Чистая `mammothToEditorHtml` ушла в существующий `docxImport.ts`. Вызовы хуков стоят ровно там, где раньше стояли их объявления, — порядок хуков не изменился | 5120 → **4632** (макс. новый — 218) | на ревью |
| 2026-07-31 | `widgets/CreateInternalCorrespondence/ui/CreateInternalCorrespondence.tsx` | **XL, срез D, часть 2.** Четыре эффекта «пришвартованных» блоков разъехались по двум файлам: `useCanvasScrollFollow.ts` (`useSideCanvasScrollFollow` — активный боковой холст, `useOriginalCanvasScrollFollow` — A4-холст входящего) и `usePaneScrollFollow.ts` (`usePanelsGroupScrollFollow`, `useNavPaneScrollFollow`); общие типы параметров — в `scrollFollowModel.ts`. `versionsLib.ts` — чистые `collectRevokedVersionIds`, `mapDocumentVersions`, `buildVersionAuthors` (три `useMemo` в компоненте сжались до вызовов). `useDraftMutations.ts` — весь узел сохранения черновика: `syncAttachmentsAfterSave`, `createDraft`, `handleDraftUpdated`, четыре варианта `updateDraft` (JSON/multipart × тихий/с тостом) и `saveDraft`; наружу отдаёт только `saveDraft` и `isSaving`. `buildEmbeddedStampHtml` (сборка data-URI SVG вшитого штампа, ~35 строк внутри `onSuccess` мутации подписи) ушла в `stampGeometry.ts`. **Важно про порядок хуков:** четыре скролл-эффекта переехали ниже объявлений рефов (`stickyHeaderRef` и др.) — раньше эффект на строке 493 закрывался на `const`, объявленный на 543, что работает только потому, что тело эффекта выполняется после рендера; передать тот же реф параметром на строке 493 нельзя (TDZ). Взаимный порядок самих эффектов сохранён | 4632 → **4267** (макс. новый — 195) | на ревью |
...
| Дата | Файл | Находка |
|---|---|---|
| 2026-08-01 | `ui/createInternalCorrespondence/useEditorClipboard.ts` | В ветке «многострочный форматированный контент» строка `fragment = buildFragmentFromHtml(sanitizeWordHtml(html));` стоит **дважды подряд** (перенесена как есть из `handleEditorPaste`). Первый фрагмент отбрасывается, поведение не меняется, но HTML буфера разбирается два раза — на большой вставке из Word это лишняя работа. Удалить второе присваивание |
| 2026-07-31 | `createInternalCorrespondence/EditorRuler.tsx` | Маркеры полей используют нативный атрибут `title` для подсказки вместо `Tooltip` (§11); сами маркеры — `div` без роли и клавиатурного управления |
| 2026-07-31 | `createInternalCorrespondence/EditorRuler.tsx` | `useEffect` с подпиской на `mousemove`/`mouseup` пересоздаётся при любом изменении `pageWidth`/полей/`onChange`, то есть на каждом кадре перетаскивания. Перенесено как есть — кандидат на перф-проход |
| 2026-07-31 | `createInternalCorrespondence/stampGeometry.ts` | `buildStampQRSvg` — мёртвый код: в репозитории нет ни одного вызова (проверено поиском по всему `src`). Вместе с ним в главном файле висел неиспользуемый импорт `generateQRMatrix`, снятый при переносе. Саму функцию не удалял (§15) |
| 2026-07-31 | `CreateInternalCorrespondence.tsx` | `let splitGroupSeq = 0` — изменяемое состояние на уровне модуля (счётчик групп авторазбиения), общий на все экземпляры компонента. Оставлен на месте намеренно: при переносе в отдельный файл счётчик остаётся синглтоном, но трогать его вместе с пагинацией опасно — выносить только вместе с логикой авторазбиения |
| 2026-07-31 | `createInternalCorrespondence/useCanvasScrollFollow.ts` | `useSideCanvasScrollFollow` и `useOriginalCanvasScrollFollow` оба пишут `transform` в `originalCanvasWrapRef`, когда включён просмотр входящего письма без сравнения версий: два эффекта считают смещение по одной и той же формуле и борются за один DOM-узел, различаясь только массивами зависимостей (у второго их больше). Похоже на исторический дубль — при разборе, скорее всего, останется один хук |
| 2026-07-31 | `createInternalCorrespondence/useDraftMutations.ts` | Четыре мутации обновления черновика (`updateDraft`, `updateDraftSilent`, `updateDraftWithFiles`, `updateDraftWithFilesSilent`) отличаются только флагом `suppressSuccessToast` и методом (`PUT` против `POST` + `_method`). Сводится к одной мутации с параметром — отдельной задачей |
| 2026-07-31 | `createInternalCorrespondence/printDocument.ts` | `buildPrintPageCss` меняет местами ширину и высоту листа при альбомной ориентации (`pageW = isLandscape ? pageHeight : pageWidth`), хотя переданные `pageWidth`/`pageHeight` уже посчитаны с учётом ориентации (`isLandscape ? 1122 : 794`). В итоге альбомная печать получает лист 794×1122 — то есть книжный. Перенесено дословно, не проверялось на реальной печати |
| 2026-07-31 | `createInternalCorrespondence/useWordImport.ts` | Ошибки импорта Word сообщаются нативным `alert()` («Поддерживаются только файлы Word», «Не удалось импортировать документ Word»), а не тостом, как весь остальной модуль (§16) |
| 2026-07-31 | `CreateInternalCorrespondence.tsx` | `UserDropdown` объявлен **внутри** тела компонента (перед `return`) — новый тип компонента на каждый рендер, значит выпадающий список получателей размонтируется и монтируется заново при любом обновлении родителя (§8, §14). Не тронут намеренно: вынос наружу сделает компонент стабильным, а это уже изменение поведения (анимации `AnimatePresence`, фокус) — не для рефакторингового PR. В `RecipientField` элемент передаётся слотом `dropdown`, поэтому текущее поведение сохранено байт-в-байт |
| 2026-07-31 | `createInternalCorrespondence/documentMeta/RecipientField.tsx` | Блоки «Кому» и «Копия» были построчными дублями (~75 строк каждый) и объединены в один компонент. При ревью сверить два места вызова: у «Кому» обёртка `px-6 pt-5 pb-4 … z-20`, у «Копии» — `px-6 pb-4 … z-10`; плейсхолдеры «Поиск получателя...» и «Поиск получателя копии...» |
| 2026-07-31 | `createInternalCorrespondence/documentMeta/AttachmentsField.tsx` | Подсказки «Просмотр вложения» / «Убрать файл» / список допустимых расширений — нативный `title`, а не `<Tooltip>` (§11). Кнопка «Прикрепить файл» также без `aria-label` |
| 2026-07-31 | `CreateInternalCorrespondence.tsx` | Пять импортов иконок (`Search`, `Plus`, `User`, `Download`, `UserPlus`) не использовались уже в HEAD до начала кампании — сняты по ходу |
| 2026-07-31 | `createInternalCorrespondence/toolbar/IncomingPagerBar.tsx` | Кнопка «Оригинал» открывает захардкоженный путь `` `/modules/correspondence/internal/incoming/${id}` `` вместо `AppRoutes` (§9). Перенесено как есть |
| 2026-07-31 | `createInternalCorrespondence/toolbar/SectionCylindersBar.tsx` | Массив-описание пяти «цилиндров» собирается прямо в JSX на каждый рендер (§14.4). Перенесён как есть — кандидат на `useMemo` в перф-проход |
| 2026-07-31 | `createInternalCorrespondence/StampPlaceholder.tsx` | Данные сертификата в плейсхолдере ЭЦП фиктивные и захардкожены: `certSerial={`SN-2026-${initials}-84201`}`, `validUntil="аз 20.03.2025 то 20.03.2026"` (плюс опечатка «то» вместо «то 20.03.2026»/«та»), `signedAt` пересчитывается через `new Date()` на каждый рендер |
| 2026-07-31 | `createInternalCorrespondence/toolbar/*`, `StampPlaceholder.tsx` | Подсказки кнопок тулбара и плейсхолдера ЭЦП — нативный атрибут `title` (через проп `title` у `TBtn` и напрямую на `div`), а не `<Tooltip>` из `@shared/ui` (§11). Пре-существующее, перенесено дословно |
| 2026-07-31 | `CreateInternalCorrespondence.tsx` | Группа боковых панелей под `panelsGroupRef` (`ApproversPanel` / `SignerPanel` / `IncomingLettersPanel` / `VersionsPanel` / `AttachmentsPanel`, ~110 строк) не вынесена: чистый pass-through потребовал бы ~45 пропсов, файл-обёртка вышел бы больше выигрыша. Выносить только вместе с объединением состояния панелей в один объект — отдельной задачей |
| 2026-07-31 | `ui/createInternalCorrespondence/` | Папка существовала в репозитории пустой (без файлов) до этого шага — похоже, заготовка от прежнего захода на декомпозицию |
