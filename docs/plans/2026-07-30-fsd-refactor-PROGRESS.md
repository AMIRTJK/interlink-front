...
- **Последнее обновление:** 2026-07-31 — Модуль `InternalCorrespondece` (ui.tsx, Editor.tsx, DocumentHeaderForm.tsx, TopNavigation.tsx) закрыт. Вне очереди идёт XL модуля 12 (`CreateInternalCorrespondence.tsx`): срезы A и B закрыты целиком (7256 → 6183), закрыт срез C целиком (липкая шапка редактора, холст листа, панель действий экрана, форма реквизитов) и часть 1 среза D (история undo/redo, перетаскивание ЭЦП, импорт Word, печать) — 6183 → **4632**, 22 новых файла, все ≤218 строк. Шаг остаётся открытым: до `return` осталось ~3940 строк логики, ядро которой — `paginateEditor` и `handleEditorKeyDown` (~900 строк вдвоём); их выносить отдельным заходом с проверкой инвариантов пагинации.
...
- [ ] `XL` 7256 → `widgets/CreateInternalCorrespondence/ui/CreateInternalCorrespondence.tsx` — под-план ниже
  ...
  - [ ] **срез D, часть 2 — что осталось в логике (~3940 строк до `return`).** Крупнейший узел — `paginateEditor` (~520 строк, один `useCallback`) и `handleEditorKeyDown` (~390): обвязка пагинации, каретки и клавиатуры. Трогать только отдельным заходом, с проверкой инвариантов из `editor-pagination-architecture`. Дальше по убыванию: кластер подписей/согласований (`signaturesConfirm` + `approvalsConfirm`, ~150), сохранение черновика (`saveDraft` + мутации, ~150), версии документа (`allVersions`, `activeVersion`, `handleSelectVersion`, ~200), скролл-синхронизация боковых канвасов (~90)
