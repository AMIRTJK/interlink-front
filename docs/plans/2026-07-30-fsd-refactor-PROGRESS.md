...
- **Последнее обновление:** 2026-07-31 — Модуль `InternalCorrespondece` (ui.tsx, Editor.tsx, DocumentHeaderForm.tsx, TopNavigation.tsx) закрыт. Вне очереди идёт XL модуля 12 (`CreateInternalCorrespondence.tsx`): срезы A и B закрыты целиком (7256 → 6183), закрыт срез C целиком (JSX) и части 1–2 среза D (история undo/redo, перетаскивание ЭЦП, импорт Word, печать, скролл-follow, версии, сохранение черновика) — 6183 → **4267**, 28 новых файлов, все ≤218 строк. Шаг остаётся открытым: до `return` осталось ~3580 строк, и это уже плотное ядро редактора — `paginateEditor`, `handleEditorKeyDown`, `insertPageBreak`/`deletePage`/вставка фрагментов (~1300 строк вместе). Их выносить только отдельным заходом с проверкой инвариантов пагинации.
